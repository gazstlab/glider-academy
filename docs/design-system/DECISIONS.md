...
| 2026-08-23 | D22 · `tokens.css` is imported from `docs/` at runtime, never copied into `src/` | §12 makes that file the single source of design values, and two hooks guard it there — `tokens-guard.sh` escalates an edit to a human and `tokens-parity.sh` keeps `tokens.json` in step; a copy under `src/` would be a third origin neither hook watches, drifting quietly until a learner sees the wrong colour. The build now depends on a path under `docs/`, so moving that file breaks it loudly, at the moment of the change | A generated copy under `src/styles/`, and a build step that writes one |
